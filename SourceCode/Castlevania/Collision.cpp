...
bool Collision::checkOnGround(Rect obj, float distance)
{
	Rect sweptRect;
...
bool Collision::isCollide(Rect myRect, Rect otherRect)
{
...
bool Collision::checkCollision(BaseObject * myEntity, BaseObject * otherEntity, float frametime, CollideDirection & direction, Rect & collideRect)
{
	float _dxEntry, _dyEntry, _dxExit, _dyExit;
	float _txEntry, _tyEntry, _txExit, _tyExit;

	Rect   myRect = myEntity->getBoundCollision();
	Rect   otherRect = otherEntity->getBoundCollision();
	Rect   broadphaseRect = getSweptBroadphaseRect(myEntity, frametime);	// là bound của object được mở rộng ra thêm một phần bằng với vận tốc (dự đoán trước bound)
...
Rect Collision::CalculateCollisionRect(Rect rect1, Rect rect2)
{
	Rect  result;
	//chon max Left
...
Rect Collision::getSweptBroadphaseRect(BaseObject * obj, float frametime)
{
	VECTOR2 velocity = VECTOR2(obj->getVelocity().x * frametime, obj->getVelocity().y * frametime);
	Rect myRect = obj->getBoundCollision();
